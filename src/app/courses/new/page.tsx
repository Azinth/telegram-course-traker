  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <form onSubmit={submit} className="space-y-3">
        ...
        />
        <textarea
          className="w-full min-h-[240px] px-3 py-2 rounded bg-gray-900 border border-gray-800"
          placeholder="Cole o índice com os módulos e #tags"
          value={indexText}
          onChange={(e) => setIndexText(e.target.value)}
        />
        {error && <div className="text-red-400 text-sm">{error}</div>}
        <button className="px-4 py-2 rounded bg-white/10 hover:bg-white/20">
          Criar
        </button>
      </form>
    </div>

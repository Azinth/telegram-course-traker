...
export async function GET(){
  ...
  return NextResponse.json(await listCoursesWithProgress(await userId(session.user.email)));
}
...
export async function POST(req: Request){
  ...
  const c = await createCourseFromIndex({ userId: uid, title: data.title, rawIndex: data.index });
